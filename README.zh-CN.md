...
- 从 S3、WebDAV、Google Cloud Storage 或 SFTP 上已有的备份对象恢复到指定 Nowledge Mem 实例。
...
- 备份可以保存到 S3/R2 兼容存储、WebDAV、Google Cloud Storage 或 SFTP，保存前可以先测试连接。
...
- Target `root_prefix`：bucket 或 WebDAV/SFTP 账号下的远端根目录/前缀。
- Task `object_prefix`：该任务自己的对象路径模板，例如 `nowledge-mem/{task}/{timestamp}`。

支持的 target 类型包括 S3/R2（`s3`）、WebDAV（`webdav`）、Google Cloud Storage（`gcs`）和 SFTP（`sftp`）。GCS 和 SFTP 使用 Afero 上游标记为实验状态的后端。GCS 可以通过 `credentials_json` 或 target 专用的 `credentials_json_env` 配置服务账号 JSON。SFTP 支持密码或私钥认证，`root_prefix` 可以填写相对路径或绝对路径；默认必须配置 `host_key_sha256`，除非显式启用 `insecure_ignore_host_key`。

路径模板变量：`{task}` / `{task_name}` 使用任务显示名称，`{task_id}` 使用内部 UUID，`{date}` 使用 UTC `YYYY-MM-DD`，`{timestamp}` 使用 UTC `YYYYMMDDTHHMMSSZ`。

自动清理远端备份时，只会扫描 `target.root_prefix + task.object_prefix` 推导出的稳定任务目录，并且只删除 `.zip` 或 `.zip.aes.json` 备份对象，不会扫描整个 bucket 或远端存储根目录。

远端恢复复用已保存的 S3/WebDAV/GCS/SFTP target 和 Nowledge Mem API source：

- 扫描对象时必须填写非空远端前缀；程序不会扫描整个 bucket 或远端存储根目录。
...
- S3/R2 存储使用 `github.com/fclairamb/afero-s3`；WebDAV 使用 `github.com/lib-x/aferodav` 和本项目的 HTTP WebDAV 适配；GCS 和 SFTP 使用 Afero 官方实验性的 `gcsfs` 和 `sftpfs` 包。
